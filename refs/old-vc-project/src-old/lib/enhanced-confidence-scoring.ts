...
/**
 * Enhanced Confidence Scoring Service
 *
 * Calculates confidence scores for enhanced analysis results including
 * external data enrichment from Exa.ai and Firecrawl APIs.
 */
export class EnhancedConfidenceScoringService {
  /**
   * Calculate enhanced confidence score with external data
   */
  static async calculateEnhancedConfidenceScore(
    ...
    try {
      ...
      // Calculate external data confidence
      const externalDataConfidence =
        this.calculateExternalDataConfidence(portfolioAnalysis);

      // Calculate overall confidence with external data weighting
      const overallConfidence =
        financialConfidence * 0.3 +
        portfolioConfidence * 0.25 +
        dataQualityConfidence * 0.25 +
        externalDataConfidence * 0.2;
      ...
      return {
        ...
        externalData: externalDataConfidence,
      };
    } catch (error) {
      console.error("Failed to calculate enhanced confidence score:", error);
      return {
        overall: 0,
        financial: 0,
        portfolio: 0,
  ...
  /**
   * Calculate confidence in external data enrichment
   */
  private static calculateExternalDataConfidence(
    portfolioAnalysis: ComprehensivePortfolioAnalysis
  ): number {
    let totalScore = 0;
    let totalFactors = 0;

    // Score company external data
    for (const company of portfolioAnalysis.companies) {
      if (company.externalData) {
        const companyScore = this.scoreExternalCompanyData(
          company.externalData
        );
    ...
    // Score sector external data
    for (const sector of portfolioAnalysis.sectors) {
      if (sector.marketData) {
        const sectorScore = this.scoreExternalSectorData(sector.marketData);
        totalScore += sectorScore;
        totalFactors += 1;
      }
    }

    return totalFactors > 0 ? totalScore / totalFactors : 0;
  }

  /**
   * Score external company data quality
   */
  private static scoreExternalCompanyData(
    externalData: ExternalCompanyData
  ): number {
    let score = 0.3; // Base score

    // Market cap data
    if (externalData.marketCap && externalData.marketCap > 0) {
      score += 0.2;
    }

    // Funding rounds data
    if (externalData.fundingRounds && externalData.fundingRounds.length > 0) {
      score += 0.2;
    }

    // Recent news data
    if (externalData.recentNews && externalData.recentNews.length > 0) {
      score += 0.1;
    }

    // Competitors data
    if (externalData.competitors && externalData.competitors.length > 0) {
      score += 0.1;
    }

    // Sector trends data
    if (externalData.sectorTrends && externalData.sectorTrends.length > 0) {
      score += 0.1;
    }
  ...
  /**
   * Score external sector data quality
   */
  private static scoreExternalSectorData(marketData: SectorMarketData): number {
    let score = 0.3; // Base score

    // Market size data
    if (marketData.marketSize && marketData.marketSize > 0) {
      score += 0.2;
    }

    // Growth rate data
    if (marketData.growthRate !== undefined) {
      score += 0.2;
    }

    // Key trends data
    if (marketData.keyTrends && marketData.keyTrends.length > 0) {
      score += 0.15;
    }
    ...
    // Market leaders data
    if (marketData.marketLeaders && marketData.marketLeaders.length > 0) {
      score += 0.05;
    }
...
/**
 * Calculate enhanced confidence score with external data
 * Main entry point for enhanced confidence scoring
 */
export async function calculateEnhancedConfidenceScore(
  financialMetrics: DetailedFinancialMetrics,
  portfolioAnalysis: ComprehensivePortfolioAnalysis,
  dataQuality: DataQualityMetrics
): Promise<ConfidenceScore> {
  return await EnhancedConfidenceScoringService.calculateEnhancedConfidenceScore(
    financialMetrics,
    portfolioAnalysis,
    dataQuality
  );
